...
class GetData:
    ...
    def get_joke(self):
        ...
        return f"Aura: {data['value']}"

    def facts(self):
        base_url = "https://uselessfacts.jsph.pl/random.json?language=en"
        data = self.get_response(base_url)
        return f"Aura: {data['text']}"

    def git_hub_info(self,github_username):
    ...
    def crypto_price(self,coin):
        base_url = f"https://api.coingecko.com/api/v3/simple/price?ids={coin}&vs_currencies=inr"
        data = self.get_response(base_url)
        if coin not in data:
            return 'Aura: Not available'
        return f"Aura: {data[coin]['inr']}"
    ...
    def get_location(self):
        ...
        data = self.get_response(base_url)
        return f"Aura: City-> {data['city']},country-> {data['country']},ISP-> {data['isp']}"

    def get_meaning(self,word):
        base_url = f"https://api.dictionaryapi.dev/api/v2/entries/en/{word}"
        data = self.get_response(base_url)
        if isinstance(data, dict):
            return "Aura: Nahi mila..."
        definition = data[0]['meanings'][0]['definitions'][0]['definition']
        return f"Aura: {word} -> {definition}"
